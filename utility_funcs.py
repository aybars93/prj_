	...
	for i in range(rows):
		for j in range(columns):	
			if(colored == True):
				...
				# if(save == True):
					# plt.imsave("image_new" +str(counter_) + ".jpg", predicted_series[counter_] )				
			else:
				axes[i, j].imshow(predicted_series[counter_].reshape(40,40), cmap="gray")
				# if(save == True):
					# plt.imsave("image_new" +str(counter_) + ".jpg", predicted_series[counter_].reshape(40,40), cmap="gray" )
					
			axes[i, j].axis('off')
			counter_ += 1
	
	if(save == True):
		plt.savefig('image_new.png')
	# plt.show()
	# plt.close()
	plt.close(fig)
