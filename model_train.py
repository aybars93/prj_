...
def getModel(filter_channels=64, output_channels=3):
	...
	x = layers.ConvLSTM2D(filters=filter_channels, kernel_size=(3, 3),  padding="same", return_sequences=True)(Input_1)
	...
	# model.compile(loss = 'binary_crossentropy', optimizer = "adam")		# adam optimizer produces good result	
	model.compile(loss = 'mse', optimizer = "adam")		# adam optimizer produces good result	
...
def train_model(path_observation_1, path_observation_2, epochs=3000, plot=False):
	model_ = getModel(32, 3)
	...
	# history = model_.fit(data_X[:100], [data_y1[:100], data_y2[:100]] , batch_size=8, epochs = epochs, verbose=2, validation_split = 0.1, callbacks=list_callbacks)
	history = model_.fit(data_X, [data_y1, data_y2] , batch_size=8, epochs=epochs, verbose=2, validation_split=0.1, callbacks=list_callbacks)
